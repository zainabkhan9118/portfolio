...
export default function KNDTerminal() {
    ...
    const themeStyles = {
        GREEN: { primary: '#4AF626', bg: '#000000', muted: '#0f2e08' },
        RED: { primary: '#ff3333', bg: '#1a0000', muted: '#4d0000' },
        AMBER: { primary: '#ffb000', bg: '#1a1200', muted: '#4d3600' },
        BLUE: { primary: '#00e5ff', bg: '#001a1f', muted: '#00334d' },
        PURPLE: { primary: '#d500f9', bg: '#1a001f', muted: '#4d004d' }
    };
    ...
    useEffect(() => {
        const lines = [
            "INITIALIZING KND PROTOCOL...",
            "LOADING ASSETS...",
            "CONNECTING TO SECURE SERVER...",
            "ACCESS GRANTED."
        ];

        // Only run boot if we are in BOOT state (strict check)
    ...
    const handleCommandSubmit = (e: React.FormEvent) => {
        ...
        switch (cmd) {
            case "HELP":
                response = "COMMANDS: OPEN [SECTION], THEME [RED/BLUE/AMBER/PURPLE/GREEN], START SIMULATION, CLEAR, WHOAMI, DATE, JOKE";
                break;
            case "THEME RED": case "SYSTEM OVERRIDE":
                setTheme("RED");
                response = "WARNING: RED ALERT MODE ACTIVATED.";
                playError();
                break;
            case "THEME AMBER": setTheme("AMBER"); response = "THEME SET: AMBER RETRO."; break;
            case "THEME BLUE": setTheme("BLUE"); response = "THEME SET: CYBER BLUE."; break;
            case "THEME PURPLE": setTheme("PURPLE"); response = "THEME SET: SYNTHWAVE PURPLE."; break;
            case "THEME GREEN": case "RESET":
                setTheme("GREEN");
                response = "SYSTEM NORMALIZED.";
                playSuccess();
                break;
            ...
            case "JOKE": response = "WHY DO JAVA DEVELOPERS WEAR GLASSES? BECAUSE THEY DON'T C#."; break;
            case "CLEAR": setCommandOutput([]); setCommandInput(""); return;
            default:
                if (cmd.startsWith("OPEN") || cmd.startsWith("GO TO")) {
                    const section = cmd.split(" ")[1];
                    const map: { [key: string]: ContentSection } = {
                        "HERO": "HERO", "HOME": "HERO",
                    ...
                    if (section && map[section]) {
                        handleMenuSelect(map[section]);
                        response = `OPENING ${section}...`;
                    } else {
                        response = `ERROR: UNKNOWN SECTION.`;
                        playError();
                    }
                } else {
    ...
    // Audio Status State for Hydration Fix
    ...
    return (
        <div
            className="terminal-wrapper h-screen w-full font-knd relative overflow-hidden flex flex-col transition-colors duration-700"
            style={{
                backgroundColor: themeStyles[theme].bg,
                color: themeStyles[theme].primary,
                '--primary': themeStyles[theme].primary,
                '--primary-foreground': themeStyles[theme].bg,
                '--muted': themeStyles[theme].muted
            } as React.CSSProperties}
        >
            <SystemDiagnostics primaryColor={themeStyles[theme].primary} />
            <MatrixRain />
            {/* Global Effects */}
            ...
            <div className="relative z-10 w-full h-full flex-1 p-2 md:p-6 flex flex-col max-w-7xl mx-auto">
                ...
                <header className="flex justify-between items-end border-b-4 border-primary pb-2 mb-4 shrink-0 bg-black/50 backdrop-blur-sm">
                    ...
                    <div className="text-right hidden md:block">
                        <div className="text-xs">SYS.TIME: {timeString}</div>
                        ...
                        <div className="relative w-full h-full flex flex-col">
                            ...
                            <div className="flex-1 overflow-y-auto p-4 md:p-6 pb-20 scrollbar-thumb-primary scrollbar-track-black">
                                ...
                                <div className="max-w-5xl mx-auto space-y-12">
                                    {activeContent === "HERO" && <TerminalHero onNavigate={handleMenuSelect} playHover={playHover} />}
                                    {activeContent === "ABOUT" && <TerminalAbout playHover={playHover} />}
                                    {activeContent === "PROJECTS" && <TerminalProjects playHover={playHover} playClick={playClick} />}
                                    {activeContent === "SKILLS" && <TerminalSkills playHover={playHover} />}
                                    {activeContent === "CONTACT" && <TerminalContact playHover={playHover} playClick={playClick} />}
                                    {activeContent === "TESTIMONIALS" && <TerminalTestimonials playHover={playHover} />}
                                </div>
                            </div>
                        </div>
                    )}
...
const TerminalHero = ({ onNavigate, playHover }: { onNavigate: (s: ContentSection) => void, playHover: () => void }) => (
...
const TerminalAbout = ({ playHover }: { playHover: () => void }) => (
...
const TerminalProjects = ({ playHover, playClick }: { playHover: () => void, playClick: () => void }) => (
    <div className="space-y-8 animate-in fade-in duration-500">
...
const TerminalSkills = ({ playHover }: { playHover: () => void }) => (
    <div className="space-y-8 animate-in fade-in duration-500">
...
const TerminalTestimonials = ({ playHover }: { playHover: () => void }) => (
    <div className="space-y-8 animate-in fade-in duration-500">
...
const TerminalContact = ({ playHover, playClick }: { playHover: () => void, playClick: () => void }) => (
